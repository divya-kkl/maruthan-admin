...
const GET_ALL_SUBCATEGORIES = gql`
  query GetAllSubCategories($search: String, $page: Int, $limit: Int) {
    getAllSubCategories(search: $search, page: $page, limit: $limit) {
      id
      name
      code
      description
...
function SubCategories() {
  ...
  const fetchSubCategories = async () => {
    try {
      setLoading(true);
      const data = await client.request(GET_ALL_SUBCATEGORIES, { search: searchTerm, page, limit });
      setFilteredSubCategories(data.getAllSubCategories || []);
      setTotalCount(data.getAllSubCategories?.length || 0);
      setError(null);
    } catch (err) {
      setError(err);
    } finally {
  ...
  const openViewDetails = (subCategory) => {
    setViewModalCategory(subCategory);
    setOpenDropdownId(null);
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingSubCategory) {
        await client.request(UPDATE_SUBCATEGORY, {
        ...
        await client.request(CREATE_SUBCATEGORY, {
          input: {
            name: formData.name,
            code: formData.code,
            description: formData.description,
  ...
  const handleDelete = async (id) => {
    setOpenDropdownId(null);
    try {
      await client.request(DELETE_SUBCATEGORY, { id });
      fetchSubCategories();
      showToast("SubCategory deleted successfully", "success");
    } catch (err) {
      const errorMessage = err.response?.errors?.[0]?.message || "Failed to delete subcategory";
  ...
  if (error) return <div className="categories-error">Error loading subcategories: {error.message}</div>;
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={subCat.id}>
                  ...
                  <td>
                    {subCat.imageUrl ? (
                      <img src={subCat.imageUrl} alt={subCat.name} className="cat-img-preview" />
                    ) : (
                  ...
                  <td>{subCat.name}</td>
                  <td>{subCat.code}</td>
                  <td>{getCategoryName(subCat.productCategoryId)}</td>
                  <td className="desc-cell">{subCat.description || "N/A"}</td>
                  <td>
                    <span className={`status-badge ${getStatusClass(subCat.status)}`}>
                      {subCat.status || 'ACTIVE'}
                    </span>
                  </td>
                  <td>{formatDateTime(subCat.createdTime)}</td>
                  <td className="relative-cell">
                    <button 
                      className="action-btn"
                      onClick={() => setOpenDropdownId(openDropdownId === subCat.id ? null : subCat.id)}
                    >
                      <FaEllipsisV />
                    </button>
                    {openDropdownId === subCat.id && (
                      <div className="dropdown-menu">
                        <button className="dropdown-item" onClick={() => openViewDetails(subCat)} style={{ display: 'flex', alignItems: 'center' }}>
                          <FaEye style={{ color: '#3b82f6', marginRight: '10px', fontSize: '15px' }} /> <span style={{ fontWeight: '600' }}>View Details</span>
                        </button>
                        <button className="dropdown-item" onClick={() => handleOpenModal(subCat)} style={{ display: 'flex', alignItems: 'center' }}>
                          <FaEdit style={{ color: '#f59e0b', marginRight: '10px', fontSize: '15px' }} /> <span style={{ fontWeight: '600' }}>Edit</span>
                        </button>
                        <button className="dropdown-item" onClick={() => handleDelete(subCat.id)} style={{ display: 'flex', alignItems: 'center' }}>
              ...
              {!loading && filteredSubCategories.length === 0 && (
                <tr>
                  <td colSpan="9" style={{ textAlign: 'center', padding: '30px', color: '#858796' }}>No subcategories found.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: '12px', padding: '30px', width: '500px', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', animation: 'slideIn 0.3s ease-out' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', borderBottom: '1px solid #f0f0f0', paddingBottom: '15px' }}>
              <h2 style={{ margin: 0, fontSize: '22px', color: '#333', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <FaEye color="#4a90e2" /> SubCategory Details
              </h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', fontSize: '15px', color: '#555' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '10px', borderBottom: '1px solid #f0f0f0', paddingBottom: '10px' }}><strong>Parent Category:</strong> <span>{getCategoryName(viewModalCategory.productCategoryId)}</span></div>
      ...
      {isModalOpen && (
        <div className="modal-overlay" onClick={handleCloseModal} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 3000, backdropFilter: 'blur(4px)' }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: '12px', padding: '30px', width: '600px', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', animation: 'slideIn 0.3s ease-out' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', borderBottom: '1px solid #f0f0f0', paddingBottom: '15px' }}>
              <h2 style={{ margin: 0, fontSize: '22px', color: '#333', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <FaEdit color="#4a90e2" /> {editingSubCategory ? "Edit SubCategory" : "Add New SubCategory"}
              </h2>
            ...
            <form onSubmit={handleSave}>
              <div style={{ marginBottom: '25px' }}>
                <h3 style={{ fontSize: '16px', color: '#4a90e2', marginBottom: '15px', borderBottom: '2px solid #4a90e2', paddingBottom: '5px', display: 'inline-block' }}>SubCategory Details</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                  <div className="form-group">
                    <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500', color: '#555', fontSize: '14px' }}>Name *</label>
                    <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="E.g. Mobiles" style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '14px' }} />
                  </div>
                  <div className="form-group">
                    <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500', color: '#555', fontSize: '14px' }}>Code *</label>
                    <input type="text" required value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value})} placeholder="E.g. MOB" style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '14px' }} />
                  </div>
                  <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                    <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500', color: '#555', fontSize: '14px' }}>Parent Category *</label>
                    <select required value={formData.productCategoryId} onChange={(e) => setFormData({...formData, productCategoryId: e.target.value})} style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '14px', backgroundColor: '#fff' }}>
                  ...
                  <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                    ...
                    <textarea rows="3" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} placeholder="SubCategory description..." style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '14px', resize: 'vertical' }}></textarea>
                  </div>
                  <div className="form-group">
              ...
              <div style={{ marginBottom: '25px' }}>
                <h3 style={{ fontSize: '16px', color: '#4a90e2', marginBottom: '15px', borderBottom: '2px solid #4a90e2', paddingBottom: '5px', display: 'inline-block' }}>SubCategory Image (Optional)</h3>
                <div style={{ border: '2px dashed #d9d9d9', borderRadius: '8px', padding: '20px', textAlign: 'center', backgroundColor: '#fafafa', position: 'relative', cursor: 'pointer', transition: 'all 0.3s' }} className="upload-zone">
                  ...
                  {uploadingImage ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
                      <img src={formData.imageUrl} alt="SubCategory" style={{ maxWidth: '100%', maxHeight: '150px', borderRadius: '6px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }} />
